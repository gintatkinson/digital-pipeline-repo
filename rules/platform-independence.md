...
- **Mermaid Relationship Label Rules**: Relationship labels containing spaces or colons MUST be enclosed in double quotes (e.g., use `RootContainer *-- NestedContainer : "contains a nested container"`, not `RootContainer *-- NestedContainer : contains a nested container`). An unquoted multi-word label is a parse error. A single unquoted word is permitted (e.g., `A --> B : references`).
