...
## Part AB — Turnkey Single-Step Installation Script Upgrade & Documentation Alignment

**STATUS: READY FOR EXECUTION.**

### AB1. Objective
Upgrade `scripts/install_pipeline.sh` to provide a true single-step turnkey installation that automatically sets up virtual environments (`.venv`), installs dependencies, copies assets, sets up git hooks, bootstraps labels, runs verification tests (`pytest`), and compiles SysML models. Update `install-guide.md` and `README.md` to document the turnkey single-step workflow and update `tests/test_install_pipeline.py` to assert the presence of all automated steps.
...
1. **Update `scripts/install_pipeline.sh`**:
   - Add automatic virtual environment creation if `.venv` does not exist:
   ...
   - Retain existing pipeline directory copying, git hook setup (`scripts/setup_git_hooks.py`), and label bootstrapping (`bootstrap_tracker_labels.py`).
   - Add automatic verification test execution if `tests/` directory exists:
     `.venv/bin/pytest tests/ 2>/dev/null || python3 -m pytest tests/ 2>/dev/null || true`
   - Add automatic SysML model compilation if `compile_sysml.py` script or `.sysml` files exist:
     `python3 scripts/compile_sysml.py docs/architecture/blueprints/*.sysml 2>/dev/null || true`

2. **Update `install-guide.md` and `README.md`**:
   - Replace multi-step quick-start breakdowns with the single-step turnkey command:
     `curl -sSL https://raw.githubusercontent.com/gintatkinson/digital-pipeline-repo/main/scripts/install_pipeline.sh | bash`
   - Document that `install_pipeline.sh` handles virtual environment creation, dependency installation, pipeline asset injection, git hook setup, label bootstrapping, test verification, and SysML compilation in a single step.

3. **Update `tests/test_install_pipeline.py`**:
   - Add assertions in `test_install_pipeline_sh_exists_and_executable` to verify `scripts/install_pipeline.sh` contains:
     - venv creation (`.venv` / `python3.12 -m venv`)
     - dependency installation (`pip install`)
     - pytest verification (`pytest`)
     - SysML model compilation triggers (`compile_sysml.py` / `sysml`)

4. **Automated Verification**:
   - Execute `python3 -m pytest tests/test_install_pipeline.py tests/test_document_references.py` and verify all tests pass cleanly.

### AB4. Verification
- `python3 -m pytest tests/test_install_pipeline.py tests/test_document_references.py`
- `python3 -m pytest tests/`

#### 4-Point Compliance Check

1. **Question or command?** Direct command to execute single-step turnkey installation upgrade and documentation alignment.
2. **Approval for a file-write this turn?** Writing `implementation_plan.md` to present the plan for user approval.
3. **Silent assumptions?** None; all requirements are explicitly specified in the user request.
4. **Does this turn write repository source or specification files?** Updating `implementation_plan.md` only; implementation edits will take place after plan approval.
