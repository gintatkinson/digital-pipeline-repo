...
When implementing adapters targeting Protobuf-backed APIs:
- **Domain Decoupling**: Generated Protobuf message classes (compiled via `protoc`) MUST NOT leak into the React UI or rendering components. They must be mapped directly to clean TypeScript domain interfaces (defined under `src/services/interfaces/`) within the adapter.
- **Asynchronous Unary RPCs**: Unary calls must be wrapped in standard TypeScript Promises returning the mapped domain interfaces.
- **gRPC Stream-to-Callback Mapping**: Streaming RPC calls must be converted to subscriber models (callback functions or RxJS Observables) in the adapter, returning a cleanup/unsubscribe closure that explicitly calls `stream.cancel()`.
- **Error Status Translation**: Adapter implementations must translate raw gRPC status codes (e.g., Code 14 - Unavailable, Code 16 - Unauthenticated) into domain-specific `Error` instances before rejecting promises or throwing stream errors.
- **Metadata Authentication**: Access credentials (such as Bearer tokens) must be dynamically injected into the metadata object of each RPC request via client interceptors or adapter helper methods.
