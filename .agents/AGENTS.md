...
2. **Dispatch a Context-Isolated Subagent**: For each item, dispatch a *fresh* subagent using whichever tool the active runtime exposes for that capability. Every one of the following is mandatory:
   ...
   - **Enforce 3-Layer Definition of Done**: The full 3-layer LUI chain — (1) Domain Model (data structure / entity), (2) ViewModel (state holder handling user actions), and (3) LUI Widget Binding + BDD User Story Widget test asserting **User Tap/Event → ViewModel Action → State Change → LUI Render** — is mandatory **per specification item**, not per subagent dispatch. The gate is `tests/test_process_discipline_gates.py::test_every_specification_has_full_lui_chain`, which unions every file carrying a given `/// Realises: [Feat-NNN/...]` tag and fails if that Feature is missing any layer. Three micro-tasks may therefore each contribute one layer to the same Feature.
